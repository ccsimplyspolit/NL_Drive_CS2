...
через MmCopyVirtualMemory). На КАЖДЫЙ кил делает три вещи:
  ...
  1. Жмёт P СРАЗУ после kill и держит ровно 2.5 секунды (статичный hold).
     Кили в течение этого окна игнорируются - это и есть cooldown.
  2. Через 2.2 секунды от P-down (т.е. за 0.3 секунды ДО отжатия P)
     нажимает клавишу из 22-клавишного tap-pool. Сама клавиша держится
     55 ms (down -> up). Таким образом yaw-tap происходит пока P
     ещё удерживается - cheat применяет MOUSE OVERRIDE в нужный момент.
  3. По истечению 2.5 секунд P отпускается.

Tap pool привязан к yaw-биндам [-35..+35] в чите (см. таблицу ниже).
Каждый kill меняет знак yaw (POS <-> NEG); внутри pool выбор
равномерный из 10 кандидатов противоположного знака, исключая
симметричный к предыдущему (никогда +18/-18 подряд - взгляд не
сводится в 0).
