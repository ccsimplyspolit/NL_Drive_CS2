...
через MmCopyVirtualMemory). На КАЖДЫЙ кил делает две вещи:

  1. Жмёт P ровно 2.5 секунды (статичный hold). Кили в течение этого
     окна игнорируются - это и есть cooldown.
  2. Один раз нажимает клавишу из 22-клавишного tap-pool, привязанного
     к yaw-биндам [-35..+35] в чите (см. таблицу ниже). Каждый kill
     меняет знак yaw (POS <-> NEG); внутри pool выбор равномерный из
     10 кандидатов противоположного знака, исключая симметричный к
     предыдущему (никогда +18/-18 подряд - взгляд не сводится в 0).
