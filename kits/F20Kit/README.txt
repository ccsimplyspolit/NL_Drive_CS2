...
!!! ВАЖНО !!!
ЗАПУСКАЙ ДРАЙВЕР ДО ЗАХОДА НА СЕРВЕР.
v16 worker НЕ делает mid-session перерезолв chain - это убирает FPS dips,
но если стартовать уже посреди матча и kills не пошли, просто перезайди
на сервер. Драйвер возьмёт baseline на следующем успешном read.
